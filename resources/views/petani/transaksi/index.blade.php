            ...
            <tbody class="table-border-bottom-0">
              ...
              <tr>
                ...
                <td class="text-center">
                  <button type="button" class="btn rounded-pill btn-info btn-sm text-white" data-bs-toggle="modal"
                    data-bs-target="#modalLihat{{ $menunggu->id }}">
                    <i class="tf-icons bx bxs-show"></i>
                    <span class="d-none d-md-inline">Lihat</span>
                  </button>
                  <div class="modal fade" id="modalLihat{{ $menunggu->id }}" aria-labelledby="modalToggleLabel"
                    tabindex="-1" style="display: none" aria-hidden="true">
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="modalToggleLabel">Lihat Transaksi</h5>
                        ...
                        <div class="modal-body text-start">
                          <table>
                            <tr>
                              ...
                              <td class="text-wrap">{{ $menunggu->tengkulak->nama }}</td>
                            </tr>
                            <tr>
                              <td class="align-top">Produk</td>
                              <td class="align-top text-center" width="20px">:</td>
                              <td class="text-wrap">{{ $menunggu->produk->nama }} ({{
                                $menunggu->produk->kategori->nama }})</td>
                            </tr>
                            @if ($menunggu->produk->kategori->nama == 'Biasa')
                            <tr>
                              <td class="align-top">Jumlah</td>
                              <td class="align-top text-center" width="20px">:</td>
                              <td class="text-wrap">{{ $menunggu->jumlah }} Kg</td>
                            </tr>
                            @endif
                            <tr>
                              <td class="align-top">Tanggal</td>
                              <td class="align-top text-center" width="20px">:</td>
                              <td class="text-wrap">{{ date('d M Y', strtotime($menunggu->created_at)) }}</td>
                            </tr>
                          </table>
                        </div>
                  ...
                  <div class="modal fade" id="modalKonfirmasi{{ $menunggu->id }}" aria-labelledby="modalToggleLabel"
                    tabindex="-1" style="display: none" aria-hidden="true">
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                          Yakin konfirmasi penawaran produk <strong>{{ $menunggu->produk->nama }}</strong> dari
                          <strong>{{ $menunggu->tengkulak->nama }}</strong>?
                        </div>
                        <div class="modal-footer">
                          <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Batal</button>
                          <a href="{{ url('transaksi/konfirmasi/' . $menunggu->id) }}" class="btn btn-primary">Ya</a>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="modal fade" id="modalBatal{{ $menunggu->id }}" aria-labelledby="modalToggleLabel"
                    tabindex="-1" style="display: none" aria-hidden="true">
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                          Yakin batalkan penawaran produk <strong>{{ $menunggu->produk->nama }}</strong> dari
                          <strong>{{ $menunggu->tengkulak->nama }}</strong>?
                        </div>
                        <div class="modal-footer">
                          <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Tutup</button>
                          <a href="{{ url('transaksi/batalkan/' . $menunggu->id) }}" class="btn btn-primary">Ya</a>
                        </div>
                      </div>
                    </div>
                  </div>
